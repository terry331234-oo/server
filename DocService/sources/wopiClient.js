...
async function getWopiFileUrl(ctx, fileInfo, userAuth) {
  ...
  if (fileInfo?.FileUrl) {
    ...
    url = `${encodeURI(userAuth.wopiSrc)}/contents?access_token=${encodeURIComponent(userAuth.access_token)}`;
...
function putFile(ctx, wopiParams, data, dataStream, dataSize, userLastChangeId, isModifiedByUser, isAutosave, isExitSave) {
  return co(function* () {
    ...
    try {
      ...
      let uri = `${encodeURI(userAuth.wopiSrc)}/contents?access_token=${encodeURIComponent(userAuth.access_token)}`;
...
function renameFile(ctx, wopiParams, name) {
  return co(function* () {
    ...
    try {
      ...
      let fileInfo = wopiParams.commonInfo.fileInfo;
      let userAuth = wopiParams.userAuth;
      let uri = `${encodeURI(userAuth.wopiSrc)}?access_token=${encodeURIComponent(userAuth.access_token)}`;
      let filterStatus = yield checkIpFilter(ctx, uri);
      if (0 !== filterStatus) {
        return res;
      }
